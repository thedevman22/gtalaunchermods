...
function WindowChrome({ children }: { children: React.ReactNode }): React.JSX.Element {
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-xl shadow-black/30">
        ...
        {profiles.map((profile) => (
          <div
            ...
            className={[
              ...
              profile.active ? 'border-accent/40 shadow-sm shadow-sky-400/20' : 'border-border'
            ].join(' ')}
          >
            ...
            <div
              className={[
                'mt-2 rounded-lg py-1.5 text-center text-[9px] font-bold uppercase tracking-wider text-bg',
                profile.active
...
function CatalogScreen(): React.JSX.Element {
  const categories = ['Visual', 'Vehicles', 'Scripts']
    ...
    <WindowChrome>
      ...
      <div className="mt-3 space-y-2">
        {categories.map((category) => (
          ...
          >
            <div className="h-10 w-14 shrink-0 rounded-md bg-gradient-to-br from-sky-500/30 to-cyan-500/20 opacity-70" />
            <div className="min-w-0 flex-1 space-y-1.5">
              <div className="h-1.5 w-2/3 rounded-full bg-elevated" />
              <p className="text-[9px] text-muted">{category}</p>
            </div>
            <span className="shrink-0 rounded-md border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[8px] font-bold uppercase text-amber-300">
              Soon
            </span>
          </div>
        ))}
